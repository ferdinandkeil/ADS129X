...
void setup() {
  ...
  ADS.WREG(ADS129X_REG_CONFIG1, ADS129X_SAMPLERATE_32); // enable 4kHz sample-rate
  ADS.WREG(ADS129X_REG_CONFIG3, (1<<ADS129X_BIT_PD_REFBUF) | (1<<6)); // enable internal reference
  ADS.WREG(ADS129X_REG_CONFIG2, 0x11);

  // setup channels
  ADS.configChannel(1, false, ADS129X_GAIN_1X, ADS129X_MUX_TEST);
  for (int i = 2; i <= 8; i++) {
    ADS.configChannel(i, false, ADS129X_GAIN_12X, ADS129X_MUX_SHORT);
  }
